...
class SearchMasterServer(port: Int, id: Int) extends AbstractSearchServer(port, id) {
  ...
  override def query(q: String) = {
    var responses = Future.collect(clients.map {client => client.query(q)}).get

    var resultString = responses.map {r => r.getContent.toString(UTF_8)}.filter(x => x.length() > 0).mkString("[", ",\n", "]")
