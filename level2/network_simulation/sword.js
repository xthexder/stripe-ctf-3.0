...
Request.prototype.handleResponse = function (res, pageData) {
  var jsonResponse,
      hmac;
      var _this = this;
  var success = function(res, pageData) {
    if (res.statusCode !== 200) {
      _this.log("Non-200 return code.");
      return false;
    }
    try {
      jsonResponse = JSON.parse(pageData);
    } catch (err) {
      _this.log("Could not parse response body.", true);
      return false;
    }
    if (!jsonResponse.hmac) {
      _this.log("Responded with no HMAC.", true);
      return false;
    }
    hmac = util.sign(_this.nonce, _this.client.simulation.secret);
    if (hmac === jsonResponse.hmac) {
      return true;
    } else {
      _this.log("Responded with invalid HMAC", true);
      return false;
    }
  }(res, pageData);

  this.simulation.registerSuccess(this.client, this.roundIndex, this.latency, success);
};
...
Simulation.prototype.runSimulation = function (clients) {
  ...
  this.simulationResults2[roundIndex] = [];
...
Simulation.prototype.registerSuccess = function (client, roundIndex, latency, success) {
  this.requestTotal += 1;

  var mouseTotal = this.simulationResults2[roundIndex][client.ip] || 0;
  this.simulationResults2[roundIndex][client.ip] = mouseTotal + 1;
  if (success) {
...
Simulation.prototype.scoreRound = function (roundResults2, index) {
  console.log("Round");
  var score = 0;
  var self = this;
  Object.keys(roundResults2).forEach(function (ip) {
    var successes = self.simulationResults[index][ip] || 0;
    var client = self.clients[ip];
    console.log("IP %s (%s): %d/%d", ip.slice(0, 8), client.type, successes, roundResults2[ip]);
    if (client.type !== "elephant") score += successes;
  });
  if (score == 0) {
...
Simulation.prototype.scoreSimulation = function () {
  ...
  var resultsPath = this.resultsPath,
      roundScores = this.simulationResults2.map(this.scoreRound, this),
  ...
  console.log("Number of total responses %s/%s", this.responseTotal, this.requestTotal)
